...
<template>
  <div class="p-3 md:p-5">
    ...
    <div class="mb-6" v-if="conference.id">
      <h3 class="text-xl font-semibold mb-3 ">Conference Information</h3>
      <Divider />
      <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="p-4 rounded-lg border border-gray-200">
          <h4 class="font-medium text-lg mb-2">Created</h4>
          <div class="flex items-center">
            <i class="pi pi-calendar text-blue-500 mr-2"></i>
            <span >{{ formatDate(conference.createdAt) }}</span>
          </div>
          <div class="flex items-center mt-2">
            <i class="pi pi-clock text-blue-500 mr-2"></i>
            <span >{{ formatTime(conference.createdAt) }}</span>
          </div>
        </div>
        <div class="p-4 rounded-lg border border-gray-200">
          <h4 class="font-medium text-lg mb-2 ">Last Updated</h4>
          <div class="flex items-center">
            <i class="pi pi-calendar text-blue-500 mr-2"></i>
            <span>{{ formatDate(conference.updatedAt) }}</span>
          </div>
          <div class="flex items-center mt-2">
            <i class="pi pi-clock text-blue-500 mr-2"></i>
            <span>{{ formatTime(conference.updatedAt) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-4 space-y-4">
        ...
        <div class="p-4 rounded-lg border border-gray-200 flex flex-column md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h4 class="font-medium ">Registration Settings</h4>
            <p class="text-sm ">Configure registration options and pricing</p>
          </div>
          <Button label="Configure" icon="pi pi-cog" severity="secondary" class="w-full md:w-auto" @click="$emit('configure-registration')" />
        </div>
        
        <div class="p-4 rounded-lg border border-red-700 flex flex-column md:flex-row md:items-center md:justify-between gap-3" v-if="conference.id">
          <div>
