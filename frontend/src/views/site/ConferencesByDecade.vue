    ...
    <div class="flex flex-col md:flex-row gap-6">
      <div class="w-full md:w-72">
        <Card class="shadow-md">
          <template #header>
            <div class="bg-primary text-white p-4 font-semibold text-lg rounded-t-lg">
              Browse by Decade
            </div>
          </template>
          <template #content>
      ...
      <div class="flex-1">
        <Card class="shadow-md">
          <template #header>
            <div v-if="selectedDecade" class="bg-primary text-white p-4 rounded-t-lg">
              <div class="flex justify-between items-center">
                <h2 class="text-2xl font-semibold">{{ selectedDecade }}s Conferences</h2>
                <Chip :label="`${conferences.length} conferences`" class="bg-primary-700" />
              </div>
            </div>
            <div v-else class="bg-primary text-white p-4 rounded-t-lg">
              <h2 class="text-2xl font-semibold">Conference Archive</h2>
            </div>
          </template>
          ...
          <template #content>
            ...
            <div v-else class="space-y-4 p-4">
              <div v-for="conference in conferences" :key="conference.id" 
                  class="border-l-4 pl-4 py-3 transition-colors rounded"
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      decades: [] as Decade[],
      selectedDecade: null as string | null,
  ...
  computed: {
    sortedDecades() {
      return [...this.decades].sort((a, b) => {
        return parseInt(b.decade) - parseInt(a.decade);
      });
    }
  },
  ...
  methods: {
    ...
    async fetchDecades() {
      ...
      try {
        const response = await conferenceService.getDecades();
        this.decades = response.data.payload;
        
        if (this.decades.length > 0) {
          const latestDecade = this.decades.reduce((latest, current) => {
            return parseInt(latest.decade) > parseInt(current.decade) ? latest : current;
          }).decade;
    ...
    async fetchConferences() {
      ...
      try {
        const response = await conferenceService.getConferencesByDecade(
          this.selectedDecade
        );
