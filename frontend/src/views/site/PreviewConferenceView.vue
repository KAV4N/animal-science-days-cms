<template>
  <div class="min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-1 py-1 pt-16">
      <!-- Loading State -->
        ...
        <Card class="w-full max-w-md mx-auto">
          <template #content>
            <div class="flex flex-col items-center space-y-1 p-1">
              ...
              <p class="text-lg font-medium">Loading preview...</p>
            </div>
          </template>
        </Card>
      </div>
        ...
        <Card class="w-full max-w-md text-center">
          <template #content>
            <div class="flex flex-col items-center space-y-1 p-1">
              ...
              <h2 class="text-2xl font-bold text-gray-900">Preview Not Available</h2>
      ...
      <!-- Main Content (Same as ConferenceView but using preview API) -->
      <div v-else-if="conference" class="space-y-1">
        ...
        <div class="bg-white rounded-lg shadow-sm p-2">
          <div class="px-1 py-1 text-center">
            ...
            <div v-if="!conference.is_published" class="mb-4">
              <Badge value="UNPUBLISHED CONFERENCE" severity="warning" class="text-sm" />
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-1 sticky top-20">
              ...
              <nav v-else-if="sortedPages?.length > 0" class="space-y-1">
                <button
                  ...
                  :class="[
                    'w-full text-left px-3 py-3 transition-colors duration-200 cursor-pointer block relative',
          ...
          <div class="lg:col-span-3 space-y-1">
            ...
            <div v-else-if="activePage" class="p-4 bg-white rounded-lg shadow-sm">
              <div class="px-1 py-1">
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <!-- Preview indicator for unpublished page -->
                    <div v-if="!activePage.is_published" class="mb-2">
                      <Badge value="UNPUBLISHED PAGE" severity="warning" class="text-sm" />
                    </div>
            ...
            <template v-if="activePage?.page_data?.length">
              ...
              >
                ...
                <div v-if="!pageData.is_published" class="absolute top-2 right-2 z-10">
                  <Badge value="UNPUBLISHED" severity="warning" class="text-xs" />
                </div>
  ...
  methods: {
    async loadConference(): Promise<void> {
      ...
      try {
        ...
        // Load pages separately using preview API
        await this.loadPages(this.slug);
    ...
    async loadPages(slug: string): Promise<void> {
      ...
      try {
        const response = await apiService.get<{ payload: PageMenu[] }>(`/v1/preview/conferences/${slug}/pages`);
        ...
        if (!this.conference) {
          this.error = err.response?.data?.message || 'Failed to load pages preview';
        }
      } finally {
    ...
    async selectPage(page: PageMenu, updateUrl: boolean = true): Promise<void> {
      ...
      try {
        ...
        console.error('Error loading page data preview:', err);
